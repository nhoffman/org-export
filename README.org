...
Additional languages may be activated using the argument =--add-langs=. In
addition, language-specific configuration may be provided as elisp expressions
via =--config= and =--config-file=. For example, to compile a document including =plantuml= code blocks (assuming a java runtime is installed):
...
Note that you may also provide language-specific configuration in an elisp code block, for example
